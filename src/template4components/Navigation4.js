                        ...
                        <ul className={displayDevice ? `${Nav4Css.navBar}` : `${Nav4Css.navBarM}`} style={(!displayDevice && activeHamburger) ? { left: "0" } : null}>
                            ...
                            <li className={`${Nav4Css.navBarLink} hover:text-black ${Nav4Css.dropdownServiceListItem}`}>SERVICES
                                <ul className={`${Nav4Css.dropdownServicesContent} list-none p-0 absolute`}>
                                    {navServices.map((item, index) => <li className={`${Nav4Css.servicesLinks} flex mt-8 mx-4 mb-0 justify-between`}><h4 className={Nav4Css.navigationLinkContent}>{item}</h4><button onClick={() => removeService(index)}><i className="ml-4 fas fa-trash-alt"></i></button></li>)}
                                    <li className={`flex mt-8 mx-4 mb-0 justify-between`}><button onClick={() => addService()}><i className="fas fa-plus"></i> ADD NEW SERVICE</button></li>
                                </ul>
                            </li>
                            <li className={`${Nav4Css.navBarLink} hover:text-black ${Nav4Css.dropdownSectorsListItem}`}>SECTORS
                                <ul className={`${Nav4Css.dropdownSectorsContent} list-none p-0 absolute`}>
                                    {navSector.map((items, indexs) => <li className={`${Nav4Css.sectorsLink} flex mt-8 mx-4 mb-0 justify-between`}><h4 className={Nav4Css.navigationLinkContent}>{items}</h4><button onClick={() => removeSector(indexs)}><i className="ml-4 fas fa-trash-alt"></i></button></li>)}
                                    <li className={`flex mt-8 mx-4 mb-0 justify-between`}><button onClick={() => addSector()}><i className="fas fa-plus"></i> ADD NEW SECTOR</button></li>
                                </ul>
                            </li>
